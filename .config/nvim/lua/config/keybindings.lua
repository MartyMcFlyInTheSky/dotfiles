...
-- Run tests
-- vim.api.nvim_set_keymap('n', '<leader>t', 'PlenaryBustedFile', { noremap = true, silent = true })

-- Quickly open tasks.json for editing
vim.keymap.set('n' , '<leader>e', function()
    local path = ".vscode/tasks.json"
    if vim.fn.filereadable(path) == 1 then
        vim.cmd.edit(path)
    else
        vim.notify("No " .. path .. " in cwd", vim.log.levels.WARN)
    end
end, { desc = "Open .vscode/tasks.json if present" })
