-- Opens a specific line on remote
-- Adapted from https://github.com/juacker/git-link.nvim/blob/main/lua/git-link/main.lua
...
---@param gitcmd string Git command to use
local function get_remote_url(gitcmd)
	local remote_url = vim.fn.trim(vim.fn.system(("%s config --get remote.origin.url"):format(gitcmd)))
	if vim.v.shell_error ~= 0 then
...
local function get_url()
	...
	vim.fn.trim(vim.fn.system(("%s rev-parse --is-inside-worktree >/dev/null 2>&1")):format(gitcmd))
	if vim.v.shell_error ~= 0 then
        -- Try fallback
        gitcmd = "git --git-dir=$HOME/.myconfig --git-work-tree=$HOME"
	    vim.fn.trim(vim.fn.system(("%s rev-parse --is-inside-worktree >/dev/null 2>&1"):format(gitcmd)))
        if vim.v.shell_error ~= 0 then
	...
	local relative_filename = vim.fn.trim(vim.fn.system(("%s ls-files --full-name %s" ):format(gitcmd, filename)))
	if vim.v.shell_error ~= 0 or relative_filename == "" then
		vim.notify("File is not tracked by git", vim.log.levels.ERROR)
		return nil
	end

	local remote_url, format_url = get_remote_url(gitcmd)
...
vim.api.nvim_create_user_command('OpenFileOnRemote', open_line_url, {})
vim.api.nvim_create_user_command('YankFileRemoteLink', copy_line_url, {})
